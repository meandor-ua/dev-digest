...
description: Defines which layer every backend file belongs to in a TypeScript + Fastify + Postgres/pgvector + Drizzle + Zod server, and the one import rule that holds those layers apart: dependencies point inward only, so Presentation and Infrastructure never import each other. Use when creating a new server module, route, service, repository, port or adapter and deciding where its code and its interface go; when a route.ts is growing SQL or business logic; when a Drizzle row type is about to leave a repository; when a service reaches for the whole Container; when adding a new external system (LLM, GitHub, git, fs, astgrep, depgraph, tokenizer, clock); when reviewing or auditing server boundaries; or on "/onion-architecture". Does not cover Fastify route/plugin mechanics (fastify-best-practices), Drizzle query syntax (drizzle-orm-patterns), Postgres schema/index design (postgresql-table-design), Zod schema authoring (zod), or TypeScript type-level details (typescript-expert).
---
